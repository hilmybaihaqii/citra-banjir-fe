...
const validateInput = (name: string, value: string) => {
  switch (name) {
    case 'name':
      ...
      return "";
    case 'phone':
      if (!/^[0-9+]+$/.test(value)) return "Hanya angka dan tanda + yang diperbolehkan.";
...
export const FeedbackForm = () => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    // PENCEGAHAN DOUBLE CLICK: Jika sedang proses, abaikan klik selanjutnya
    ...
    // Mulai proses loading, tombol akan di-disable
    setSubmitting(true);
    ...
    try {
      ...
      // Cek apakah response berupa JSON yang valid sebelum di-parse
      const contentType = response.headers.get("content-type");
      if (!contentType || !contentType.includes("application/json")) {
         throw new Error("Server tidak merespon dengan format JSON. Periksa koneksi atau URL API Anda.");
      }
      ...
      // Fix untuk TypeScript error 'any'
      if (error instanceof Error) {
        setApiError(error.message);
      } else {
        setApiError('Terjadi kesalahan pada server. Silakan coba beberapa saat lagi.');
      }
    } finally {
      // Selesai memproses, kembalikan state (agar tombol bisa ditekan lagi jika gagal)
      setSubmitting(false); 
    }
  };
      ...
      {status === 'success' ? (
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-x-12">
            ...
            <FloatingInput label="Nomor Telepon / WhatsApp" name="phone" type="tel" value={form.phone} onChange={handleChange} error={errors.phone} />
          </div>
          ...
          <div className="flex flex-col items-end gap-6 pt-6">
            <AnimatePresence>
              {(securityError || apiError || Object.values(errors).some(e => e)) && (
                 <motion.div initial={{opacity:0, y: 10}} animate={{opacity:1, y:0}} exit={{opacity:0}} 
                   className="text-xs font-medium text-rose-500 flex items-center gap-2 bg-rose-50 px-4 py-3 rounded-md border border-rose-100">
                    <AlertCircle size={14} /> 
                    {apiError ? apiError : "Mohon lengkapi formulir & verifikasi keamanan dengan benar."}
                 </motion.div>
              )}
            </AnimatePresence>
