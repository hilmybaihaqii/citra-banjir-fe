...
const MapInformasi = () => {
  const centerPosition: [number, number] = [-6.99, 107.63];
  ...
  const [activeModal, setActiveModal] = useState<"data" | "dampak" | null>(
    null,
  );
  ...
  const styleKabupaten = showKabupaten
    ? { ...baseBoundaryStyle, color: "#000000", dashArray: "5, 5" }
    : transparentStyle;
  const styleKecamatan = showKecamatan
    ? { ...baseBoundaryStyle, color: "#000000", dashArray: "3, 6" }
    : transparentStyle;
  ...
  return (
    <div className="w-full h-full relative font-sans overflow-hidden bg-slate-50">
      ...
      >
        ...
        {!isLoading &&
          mapLocations.map((region) => (
            <Marker
              key={region.id}
              position={[region.latitude, region.longitude]}
              icon={getCustomIcon(region.alertStatus)}
            ...
            >
              <Popup closeButton={false} className="custom-leaflet-popup">
                <div className="p-1 min-w-35">
                  <h3 className="font-bold text-slate-800 text-[11px] mb-1.5 uppercase tracking-widest text-center">
                    {region.regionName}
                  </h3>
                  <div className="flex items-center justify-center gap-2 text-[10px] text-slate-500">
                    <span
                      className={`px-2 py-0.5 rounded-sm text-[9px] font-bold text-white uppercase tracking-widest ${region.alertStatus === "RAWAN_BANJIR" ? "bg-rose-600" : "bg-emerald-500"}`}
                    >
