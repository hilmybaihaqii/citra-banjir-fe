...
export default function AdminSettings() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      if (savedUser) {
        ...
        // Fallback default untuk developer pusat
        const defaultUser = {
          username: "root",
  ...
  const handleSaveProfile = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setSuccessMessage("Profil Pusat berhasil diperbarui!");
  ...
  const handleSavePassword = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setSuccessMessage("Password administrator diperbarui!");
      setTimeout(() => setSuccessMessage(""), 3000);
    }, 1000);
  };
  ...
  return (
    <div
      className={`mx-auto flex w-full max-w-xl flex-col pb-12 lg:pb-8 ${outfit.className}`}
    >
      {/* Toast Success */}
      <div
        className={`fixed left-1/2 top-6 z-100 -translate-x-1/2 rounded-xl border border-emerald-200 bg-white px-6 py-4 shadow-2xl transition-all duration-300 ${successMessage ? "translate-y-0 opacity-100" : "pointer-events-none -translate-y-8 opacity-0"}`}
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-bold uppercase text-slate-700">
                {AGENCIES[userData.agency_id] || "Citra Banjir Pusat"}
              </p>
            </div>
          </div>
        </div>
