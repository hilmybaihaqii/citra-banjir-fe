...
export default function UpdateWilayahPage() {
  ...
  // State Awal yang sudah ditambahkan parameter baru
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      localStorage.setItem("simulasi_database_banjir", JSON.stringify(KECAMATAN_DATA));
    }
  }, []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.lat || !formData.lng) { setErrorMessage("Silakan klik pada Peta untuk menentukan Koordinat!"); return; }
    ...
    setTimeout(() => {
      try {
        ...
        localStorage.setItem("simulasi_database_banjir", JSON.stringify(updatedLocations));
          ...
          <form id="updateRegionForm" onSubmit={handleSubmit} className={`flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar ${editingId ? 'bg-amber-50/30' : 'bg-white'}`}>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <input required type="text" name="lat" value={formData.lat} onChange={handleInputChange} placeholder="Latitude (Klik Peta)" className="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-sm text-xs font-medium text-slate-700 outline-none read-only:bg-slate-100" readOnly />
                <input required type="text" name="lng" value={formData.lng} onChange={handleInputChange} placeholder="Longitude (Klik Peta)" className="w-full px-3 py-2 bg-slate-100 border border-slate-200 rounded-sm text-xs font-medium text-slate-700 outline-none read-only:bg-slate-100" readOnly />
              </div>
            </div>
