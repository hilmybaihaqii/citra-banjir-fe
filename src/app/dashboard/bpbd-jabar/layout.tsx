...
export default function BPBDLayout({
  ...
  return (
    <div className={`flex h-dvh w-full overflow-hidden bg-slate-50 ${outfit.className}`}>
      ...
      <aside 
        className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-blue-950 text-white shadow-2xl transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 ${
      ...
      >
        <div className="flex h-20 shrink-0 items-center justify-between border-b border-white/10 px-6">
          ...
          <button 
            className="rounded-lg p-1.5 text-slate-300 hover:bg-white/10 hover:text-white lg:hidden transition-colors" 
            onClick={() => setIsSidebarOpen(false)}
          >
            <X size={24} />
          </button>
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto p-4 custom-scrollbar">
          ...
          <Link href="/dashboard/bpbd-jabar" onClick={handleMenuClick} className="block">
          ...
          <Link href="/dashboard/bpbd-jabar/laporan" onClick={handleMenuClick} className="block">
          ...
          <Link href="/dashboard/bpbd-jabar/update-wilayah" onClick={handleMenuClick} className="block">
          ...
          <div className="mt-6 border-t border-white/10 pt-4">
            ...
            {userData?.role === "superadmin" && (
              <Link href="/dashboard/bpbd-jabar/manajemen-user" onClick={handleMenuClick} className="block mb-1.5">
            ...
            <Link href="/dashboard/bpbd-jabar/log-aktivitas" onClick={handleMenuClick} className="block mb-1.5">
              <button className={isActive("/dashboard/bpbd-jabar/log-aktivitas") ? activeClass : inactiveClass}>
                <div className="flex items-center gap-3">
                  <History size={18} /> Log Aktivitas
                </div>
                {isActive("/dashboard/bpbd-jabar/log-aktivitas") && <ChevronRight size={14} />}
              </button>
            </Link>
            
            <Link href="/dashboard/bpbd-jabar/pengaturan" onClick={handleMenuClick} className="block">
      ...
      <div className="flex flex-1 flex-col min-w-0">
        ...
        <header className="flex h-20 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 shadow-sm lg:justify-end lg:px-8 z-10">
          <button 
          ...
          <div className="flex items-center gap-3 lg:gap-5">
            ...
            <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 p-1 pr-3 lg:pr-4 transition-colors hover:border-amber-400">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50/50 p-4 lg:p-8 custom-scrollbar">
          {children}
        </main>
      </div>
