...
export default function BPBDLayout({
      ...
      try {
        ...
        if (decodedToken && decodedToken.exp) {
          ...
          if (timeLeft <= 0) {
            ...
            // Jika token MASIH BERLAKU, pasang timer auto-logout
  ...
  const isActive = (path: string) => {
    if (
      ...
      pathname === "/dashboard/bpbd-jabar"
    )
      return true;
    return path !== "/dashboard/bpbd-jabar" && pathname.startsWith(path);
  };
      ...
      >
        ...
        <nav className="flex-1 space-y-1.5 overflow-y-auto p-4 custom-scrollbar">
          ...
          >
            <button
              className={
                isActive("/dashboard/bpbd-jabar") ? activeClass : inactiveClass
              }
            >
          ...
          <Link
            href="/dashboard/bpbd-jabar/laporan"
            className="block"
            onClick={() => setIsSidebarOpen(false)}
          >
            <button
              className={
                isActive("/dashboard/bpbd-jabar/laporan")
                  ? activeClass
                  : inactiveClass
              }
            >
          ...
          <Link
            href="/dashboard/bpbd-jabar/update-wilayah"
            className="block"
            onClick={() => setIsSidebarOpen(false)}
          >
            <button
              className={
                isActive("/dashboard/bpbd-jabar/update-wilayah")
                  ? activeClass
                  : inactiveClass
              }
            >
          ...
          <div className="mt-6 border-t border-white/10 pt-4">
            ...
            {userData?.role === "MASTER_ADMIN" && (
              ...
              >
                <button
                  className={
                    isActive("/dashboard/bpbd-jabar/manajemen-user")
                      ? activeClass
                      : inactiveClass
                  }
                >
            ...
            <Link
              href="/dashboard/bpbd-jabar/log-aktivitas"
              className="block mb-1.5"
              onClick={() => setIsSidebarOpen(false)}
            >
              <button
                className={
                  isActive("/dashboard/bpbd-jabar/log-aktivitas")
                    ? activeClass
                    : inactiveClass
                }
              >
            ...
            <Link
              href="/dashboard/bpbd-jabar/pengaturan"
              className="block"
              onClick={() => setIsSidebarOpen(false)}
            >
              <button
                className={
                  isActive("/dashboard/bpbd-jabar/pengaturan")
                    ? activeClass
                    : inactiveClass
                }
              >
