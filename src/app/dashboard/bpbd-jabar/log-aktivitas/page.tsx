...
export default function BPBDJabarLogsPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      console.error("Gagal sinkronisasi log BPBD Jabar");
    } finally {
      setIsLoading(false);
    }
  }, []);
      ...
      <div className="flex shrink-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between px-4 sm:px-0">
        <div>
          <h1 className="text-xl font-black uppercase tracking-tight text-blue-950 md:text-2xl">
            Log Aktivitas BPBD Jabar
          </h1>
          <p className="mt-1 text-sm font-medium tracking-wide text-slate-500">
            Riwayat perubahan data dan aksi personil BPBD Provinsi Jawa Barat.
          </p>
        </div>
        ...
        <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center lg:w-auto">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex flex-1 min-w-35 items-center justify-center gap-2 rounded-md bg-emerald-600 px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-white shadow-sm hover:bg-emerald-700 disabled:opacity-70 sm:w-auto"
            >
              {isExporting ? (
