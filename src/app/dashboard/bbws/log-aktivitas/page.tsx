...
// Interface disesuaikan dengan payload API Backend (serupa dengan BPBD & BMKG)
interface LogData {
  id: number;
  action: string;
  description: string;
...
export default function LogAktivitasBBWSPage() {
  ...
  // 1. FETCH DATA LOGS DARI API KHUSUS BBWS
  const fetchLogs = useCallback(async () => {
    setIsLoading(true);
    try {
      const token = Cookies.get("auth_token");
      const res = await fetch(`${API_URL}/bbws/logs`, {
        method: "GET",
        headers: {
      ...
      showToast("Gagal memuat riwayat aktivitas BBWS.", "error");
    } finally {
      setIsLoading(false);
    }
  }, [API_URL]);
  ...
  // 2. EXPORT EXCEL DARI API BBWS
  const handleExportExcel = async () => {
    setIsExporting(true);
    try {
      const token = Cookies.get("auth_token");
      const res = await fetch(`${API_URL}/bbws/logs/export`, {
        method: "GET",
        headers: {
          ...(token ? { Authorization: `Bearer ${token}` } : {}),
        },
      ...
      a.href = url;
      a.download = `Log_Aktivitas_BBWS_Citarum_${new Date().getTime()}.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      window.URL.revokeObjectURL(url);

      showToast("Berkas Excel BBWS berhasil diunduh!", "success");
    } catch (error) {
  ...
  // FORMAT WARNA BADGE (Termasuk custom action BBWS)
