...
export default function BPBDKabLogsPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      console.error("Gagal sinkronisasi log BPBD Kabupaten");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleExportExcel = () => {
    ...
    setTimeout(() => {
      ...
      alert(
        "Berkas Excel 'Log_Aktivitas_BPBD_Kab_Bandung.xlsx' berhasil diunduh!",
      );
    }, 2000);
  };
      ...
      <div className="flex shrink-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between px-4 sm:px-0">
        <div>
          ...
          <p className="mt-1 text-sm font-medium tracking-wide text-slate-500">
            Riwayat perubahan data dan aksi personil Pusdalops BPBD Kabupaten
            Bandung.
          </p>
        </div>
        ...
        <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center lg:w-auto">
          ...
          <div className="flex gap-2">
            <button
              onClick={fetchLogs}
              className="flex items-center justify-center rounded-md border border-slate-300 bg-white p-2.5 text-slate-600 hover:bg-slate-50 transition-colors"
            >
              <RefreshCw
                size={18}
            ...
            <button
              ...
              className="flex flex-1 min-w-35 items-center justify-center gap-2 rounded-md bg-emerald-600 px-5 py-2.5 text-xs font-bold uppercase tracking-widest text-white shadow-sm transition-colors hover:bg-emerald-700 disabled:opacity-70 sm:w-auto"
            >
              {isExporting ? (
