...
export default function BMKGWilayahPage() {
  const [isMounted, setIsMounted] = useState(false);
  const [rawRegions, setRawRegions] = useState<RegionData[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isSubmitting, setIsSubmitting] = useState(false);

  // PERBAIKAN: Mengizinkan tipe string atau number
  ...
  const handleEditRegion = (region: RegionData) => {
    setEditingId(region.id); // Sekarang aman karena menerima string | number
    setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (editingId) {
        ...
        const newId = `region-${Math.floor(Math.random() * 10000)}`;
        setRawRegions((prev) => [
          ...prev,
          {
            id: newId,
  ...
  // PERBAIKAN: Parameter id menerima string | number
      ...
      <div className="z-10 flex shrink-0 flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <h1 className="text-xl md:text-2xl font-black uppercase tracking-tight text-blue-950">
            Wilayah Pantauan (BMKG)
          </h1>
          <p className="mt-1 text-sm font-medium tracking-wide text-slate-500">
            Daftarkan dan kelola titik koordinat wilayah kerja pemantauan iklim
            & cuaca.
          </p>
        </div>
      </div>
        ...
        <div className="relative z-0 flex h-100 w-full shrink-0 flex-col overflow-hidden rounded-xl border border-slate-200 lg:h-full lg:w-2/3 shadow-sm">
          {isLoading ? (
            ...
            <MapPicker
              ...
              // PERBAIKAN: Memastikan parameter (id) diteruskan dengan aman sebagai string | number
              onDeleteRegion={(id: string | number) =>
                handleDeleteRegion(
                  id,
            ...
            >
              <div className="bg-blue-50 border border-blue-100 rounded-md p-3 mb-2">
                <p className="text-[10px] font-medium text-blue-800 leading-snug">
                  Gunakan form ini untuk mendaftarkan titik koordinat. Data
                  meteorologi diinput melalui menu Data Pemantauan.
                </p>
              </div>
