...
const CustomNumberInput = ({
  ...
  return (
    <div className="flex flex-col gap-1.5 mb-4">
      <label
        className={`text-xs font-semibold ${disabled ? "text-slate-400" : "text-slate-700"}`}
      >
        {label}
      </label>
      <div
        className={`flex items-center justify-between border rounded px-3 py-2.5 transition-all shadow-sm ${
          disabled
      ...
      >
        <input
          ...
          disabled={disabled}
          className="bg-transparent w-full font-bold text-sm outline-none text-slate-900 disabled:text-slate-400"
          placeholder="0,00"
        />
        <div className="flex items-center gap-2 shrink-0 select-none ml-2">
          ...
          <button
            ...
            disabled={disabled}
            className="w-6 h-6 flex items-center justify-center rounded bg-slate-100 text-slate-600 hover:bg-slate-200 border border-slate-200 font-bold text-base leading-none"
          >
            −
          </button>
          <button
...
export default function DataPantauanPage() {
  ...
  const fetchMasterData = useCallback(async () => {
    ...
    try {
      ...
      showToast("Gagal memuat sinkronisasi riwayat pantauan.", "error");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const confirmDelete = async () => {
    try {
      ...
      if (!response.ok)
        throw new Error("Gagal menghapus log dari server database");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = `${API_BASE_URL}/pemantauan-terpadu/${Number(editingId)}`;
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData?.message || "Gagal mengirim data pemantauan terpadu",
        );
      }

      showToast(
        "Data cuaca BMKG berhasil diperbarui ke dalam sistem.",
        "success",
      );
      ...
      const msg =
        error instanceof Error
          ? error.message
          : "Terjadi kesalahan sistem saat memproses database.";
      showToast(msg, "error");
    } finally {
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {toast.show && (
          <motion.div
            ...
            className={`fixed z-200 flex items-center gap-3 rounded-lg border px-4 py-3.5 shadow-2xl backdrop-blur-md bottom-20 left-4 right-4 sm:bottom-10 sm:left-auto sm:right-10 sm:max-w-md ${toast.type === "success" ? "border-emerald-200/60 bg-emerald-50/95 text-emerald-700" : "border-rose-200/60 bg-rose-50/95 text-rose-700"}`}
          >
            {toast.type === "success" ? (
      ...
      <div className="flex shrink-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between px-4 sm:px-0">
        <div>
          <h1 className="text-xl font-black uppercase tracking-tight text-blue-950 md:text-2xl">
            Data Pemantauan Terpadu (Sektor BMKG)
          </h1>
          <p className="mt-1 text-sm font-medium tracking-wide text-slate-500">
            Lengkapi hasil parameter cuaca lapangan pada entri data log
            hidrologi BBWS Citarum.
          </p>
        </div>
        {editingId && (
          <button
      ...
      >
        <div
          className={`bg-white border rounded-md p-5 shadow-sm transition-all ${editingId ? "border-amber-400 shadow-amber-400/20" : "border-slate-200"} mx-4 sm:mx-0`}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <select
                ...
                required
                disabled={true}
                className="w-full border border-slate-300 rounded px-3 py-2.5 text-xs font-bold outline-none bg-slate-100 border-slate-200 text-slate-400 cursor-not-allowed"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start mx-4 sm:mx-0">
          ...
          <div className="flex flex-col gap-6">
            {/* SEKTOR BMKG */}
            <div
              className={`bg-white border rounded overflow-hidden shadow-sm transition-all ${editingId ? "border-amber-400" : "border-slate-200"}`}
            >
              <div className="bg-slate-50 border-b border-slate-200 px-4 py-3">
                <h2 className="text-xs font-black text-slate-800 uppercase tracking-widest">
                  Data Cuaca (Sektor BMKG)
                </h2>
              </div>
              <div className="p-4 space-y-4">
                <CustomNumberInput
            ...
            <div className="bg-white border border-slate-200 rounded p-4 shadow-sm flex flex-col gap-2">
              <p className="text-[10px] text-slate-400 text-center leading-snug font-medium uppercase tracking-wider">
                Pilih data dari tabel di bawah lalu klik Edit untuk memperbarui
                parameter.
              </p>
              <button
                type="submit"
              ...
              >
                ...
                {isSubmitting ? "MEMPROSES..." : "PERBARUI DATA METEOROLOGI"}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm mx-4 sm:mx-0">
        <div className="bg-slate-50 border-b border-slate-200 px-4 py-3 flex items-center gap-2">
          ...
          <h2 className="text-xs font-black text-slate-800 uppercase tracking-widest">
            Riwayat Pantauan & Prediksi Model
          </h2>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {monitoringLogs.length === 0 ? (
                <tr>
                  ...
                  >
                    Belum ada riwayat data pemantauan.
                  </td>
                </tr>
              ) : (
                    ...
                    <td className="p-4 text-slate-600 font-medium text-xs">
                      <div className="flex flex-col gap-1">
                        <span>
                          Suhu:{" "}
                          <b
                            className={`${log.suhu === "0,00" ? "text-red-600" : "text-slate-900"}`}
                          >
                            {log.suhu} °C
                          </b>
                        </span>
                        <span>
                          Angin:{" "}
                          <b
                            className={`${log.kecepatanAngin === "0,00" ? "text-red-600" : "text-slate-900"}`}
                          >
