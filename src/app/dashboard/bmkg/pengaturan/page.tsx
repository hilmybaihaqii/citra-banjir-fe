...
export default function BMKGSettingsPage() {
  ...
  const handleSaveName = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // GANTI: Perbarui data nama di cookie agar langsung efek ke header/sidebar dashboard
  ...
  if (isLoadingData || !userData) {
    return (
      <div className={`flex h-full w-full items-center justify-center ${outfit.className}`}>
        <LoadingSpinner message="Memuat profil BMKG..." />
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between rounded-sm border border-slate-200 bg-slate-50 p-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-slate-700">
                {userData.agency ? (AGENCIES[userData.agency] || userData.agency) : "BMKG Jawa Barat"}
              </p>
            </div>
          </div>
          <Lock size={14} className="text-slate-300" />
