...
import { AddUserBMKGModal } from "@/components/ui/modal/AddUserBMKGModal";
...
export default function ManajemenUserBMKG() {
  ...
  const handleDeleteUser = async () => {
    ...
    try {
      ...
      console.error(error);
      showToast(
        "Gagal menghapus pengguna. Anda mungkin tidak memiliki izin.",
        "error",
      );
      throw error;
    }
  ...
  return (
    <div className="flex flex-col gap-6 p-4 pb-12 sm:p-6 lg:pb-8 w-full max-w-350 mx-auto relative animate-in fade-in duration-500">
      ...
      {/* ALERT MODE AKSES */}
      {!isMasterAdmin && (
        <div className="flex shrink-0 items-start gap-3 rounded-xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
          ...
          <div>
            ...
            <p className="mt-1 text-xs font-medium text-amber-800 leading-relaxed max-w-3xl">
              Anda login sebagai Petugas BMKG biasa. Penambahan dan penghapusan
              akun dibatasi khusus untuk Master Admin/Kepala Stasiun BMKG.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="flex shrink-0 flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-medium tracking-wide text-slate-500">
            Daftar akses akun internal operasional BMKG Provinsi Jawa Barat.
          </p>
        </div>
        ...
        <div className="flex w-full flex-col gap-3 sm:flex-row sm:items-center lg:w-auto">
          ...
          {isMasterAdmin && (
            <button
              ...
              className="flex w-full shrink-0 items-center justify-center gap-2 rounded-md bg-blue-950 px-6 py-2.5 text-xs font-bold uppercase tracking-widest text-white shadow-sm transition-colors hover:bg-blue-900 active:scale-[0.98] sm:w-auto"
            >
          ...
          <table className="w-full min-w-200 border-collapse text-left">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="w-40 p-4 text-center text-[10px] font-black uppercase tracking-widest text-slate-500">
                  Level Akses
                </th>
                {isMasterAdmin && (
                ...
                filteredUsers.map((user, index) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      ...
                      <p className="mt-0.5 text-[11px] font-bold tracking-widest text-slate-400">
                        {user.email}
                      </p>
                    </td>
                    <td className="p-4">
                      <span className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest text-slate-600">
                        {user.agency || "BMKG Jawa Barat"}
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      <span
                        className={`inline-flex items-center justify-center min-w-22.5 rounded-sm px-2 py-1.5 text-[9px] font-black uppercase tracking-widest shadow-sm ${
                    ...
                    {isMasterAdmin && (
                      <td className="p-4 text-center">
                        <button
                          ...
                          className="mx-auto flex h-9 w-9 items-center justify-center rounded-md bg-rose-50 text-rose-600 shadow-sm transition-all hover:bg-rose-600 hover:text-white disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-rose-50 disabled:hover:text-rose-600 active:scale-95"
                          disabled={
                            user.role === "MASTER_ADMIN" ||
                            user.role === "SUPER_ADMIN" ||
