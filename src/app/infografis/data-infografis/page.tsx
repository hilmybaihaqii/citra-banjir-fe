...
export default function InfographicPage() {
  ...
  const totalLokasi = locations.filter(loc => loc.alertStatus === "RAWAN_BANJIR").length;
  ...
  const currentDateText = new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pb-24 pt-4 md:pt-10">
        ...
        <motion.div initial="hidden" animate="show" variants={containerVar} className="mb-12 md:mb-16 border-b border-slate-200 pb-8 md:pb-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="overflow-hidden">
              ...
              {/* DATE RANGE PICKER */}
              <motion.div variants={itemVar} className="flex flex-col sm:flex-row items-start sm:items-center gap-3 mt-6">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">s/d</span>
                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-md px-3 py-2 shadow-sm focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all">
                  <Calendar size={14} className="text-slate-400" />
                  <input 
                    type="date" 
            ...
            <motion.div variants={itemVar} className="flex flex-col gap-4 text-left md:text-right items-start md:items-end">
               <div className={`flex items-center gap-2 px-3 py-1.5 border rounded-sm ${
                 isRawan ? 'border-rose-200 bg-rose-50 text-rose-700' : 'border-emerald-200 bg-emerald-50 text-emerald-700'
               }`}>
                 <span className="relative flex h-2 w-2">
                   <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isRawan ? 'bg-rose-400' : 'bg-emerald-400'}`}></span>
                   <span className={`relative inline-flex rounded-full h-2 w-2 ${isRawan ? 'bg-rose-600' : 'bg-emerald-600'}`}></span>
                 </span>
               ...
               <p className="text-slate-500 max-w-sm text-xs md:text-sm leading-relaxed">
                Data real-time disinkronisasi dari pantauan lapangan BPBD.<br/>
                {!startDate && !endDate ? `Per: ${currentDateText}` : 'Menampilkan data sesuai rentang waktu.'}
              </p>
            </motion.div>
          </div>
        </motion.div>
        ...
        {isLoading ? (
          <div className="h-64 w-full flex flex-col items-center justify-center gap-4">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Menganalisis Data...</span>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24">
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.2, ease: [0.16, 1, 0.3, 1] }} className="lg:col-span-4 space-y-12">
                ...
                <div className="bg-slate-900 text-white p-6 md:p-8 border border-slate-800 relative overflow-hidden group rounded-sm shadow-xl">
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-sm text-slate-400 leading-relaxed mb-8">
                      Sebanyak estimasi {formatCommas(aggregateStats.jiwaTerdampak)} jiwa ({formatCommas(aggregateStats.kepalaKeluarga)} KK) terdampak luapan air pada periode ini.
                    </p>
                  ...
                  <ul className="space-y-3">
                    {chartData.map((item, i) => (
                      <li key={i} className="flex justify-between items-center text-sm">
                        ...
                        <span className="font-mono text-slate-500 bg-slate-50 px-2 py-0.5 rounded shrink-0">{formatCommas(item.value)} Jiwa</span>
                      </li>
                    ))}
                    {chartData.length === 0 && (
                      <li className="text-xs text-slate-400 italic">Tidak ada data wilayah pada rentang waktu ini.</li>
                    )}
                  </ul>
                </div>
            </motion.div>
                    ...
                    <div className="h-72 w-full">
                      {chartData.length > 0 ? (
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center border border-dashed border-slate-200 rounded-lg gap-2">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Data Grafik Tidak Tersedia</span>
                        </div>
                      )}
                    </div>
                </motion.div>
